 ...
 */
class MqttAndroidToken implements IMqttToken {
    ...
    /**
     * Make sure MqttToken to run on the  Android Thread
     */
     ...
     */
    MqttAndroidToken(MqttAndroidClient client,
    ...
    void notifyComplete() {
        synchronized (waitObject) {
            ...
            handler.post(() -> {
                if (listener != null) {
                    listener.onSuccess(MqttAndroidToken.this);
                }
            });
        }
    }
    ...
    void notifyFailure(Throwable exception) {
        synchronized (waitObject) {
            ...
            handler.post(() -> {
                if (listener != null) {
                    listener.onFailure(MqttAndroidToken.this, exception);
                }
            });
        }
